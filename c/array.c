...
int main_array(void)
{
    ...
    // 여러 개의 변수를 함께, 동시에 생성
    // int subway_array[3];
    // subway_array[0] = 30; // index가 0부터 시작
    // subway_array[1] = 40;
    // subway_array[2] = 50;
    ...
    // int arr[10] = {1, 2}; // 세 번째 값부터는 자동으로 0으로 초기화됨.
    // for (int i = 0; i < 10; i++)
    // {
    //     printf("%d\n", arr[i]);
    // }

    // int arr[] = {1, 2}; // arr[2] 와 동일하다.
    ...
    // char c_array[6] = {'c', 'o', 'd', 'i', 'n', 'g'}; // 마지막에 이상한 문자가 들어간다.
    // printf("%s\n", c_array);

    // char c_array[10] = {'c', 'o', 'd', 'i', 'n', 'g'}; // 남는 공간이 있으면 문자열 끝이 자동으로 들어간다.
    // printf("%s\n", c_array);
    // for (int i = 0; i < sizeof(c_array); i++)
    // {
    //     printf("%d\n", c_array[i]); // 만약 %d로 출력한다면, ASCII 코드 값이 출력된다. (NULL 문자는 0으로 입력됨)
    // }
